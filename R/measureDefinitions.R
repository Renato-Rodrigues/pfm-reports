  ...
  defs <- rbind(
    ...
    d("Selection", "gatePass", "selection, model-selection",
      paste0("TRUE only if the spec clears every HARD gate in BOTH sectors: maxVIF < 10, model ",
             "converged, no unintended lagged term, ΔR² ≤ 1, and (stringency) no trend dominance ",
             "(trendShare ≤ 0.5, ADR 0033). gatePass = FALSE specs rank last.")),
    ...
    d("Selection", "trendShare", "selection",
      paste0("Share of the fitted linear-predictor VARIANCE carried by the (atheoretical) time-trend ",
             "term. A HARD GATE rejects specs with trendShare > 0.5 in any sector (ADR 0033 — the ",
             "trend explains more than everything else combined). Stringency-only (the adoption ",
             "trend is off, ADR 0010).")),
    d("Selection", "nonTheoryShare", "selection",
      paste0("Share of the fitted linear-predictor variance from the NON-theory, non-FE terms ",
             "(controls + time trend), both stages. A DIAGNOSTIC (reported, not gated, ADR 0033): ",
             "high = an atheoretical-signal spec. Controls are legitimate confounders, so this is ",
             "surfaced for inspection, never auto-rejected.")),
    ...
    d("Projection", "Projection Sanity gate", "selection, results-stringency, publication",
      paste0("Scenario screen run after maximin. SEVERE (disqualifying): price > 2000 USD/tCO2, ",
             ...
             "policy), seam jumps, price spikes, and Pmax-pinning for the saturating form (ADR 0032: ",
             "saturating specs are bounded by Pmax, so they BYPASS the extrapolation clamp / ",
             "clamp-reliance and carry a non-severe `pmax-pinned` indicator instead). Evaluated in ",
             "expanding batches; first PASS wins.")),
